...
(deftest exec-test
  (expecting "returns true on success"
    (expect true (share-channels/exec fake-list-channels fake-post-message)))

  (expecting "returns true even with empty channels"
    (expect true (share-channels/exec fake-empty-list-channels fake-post-message)))

  (expecting "calls list-channels"
    (let [fake-list-channels-spy (spy fake-list-channels)]
      (share-channels/exec fake-list-channels-spy fake-post-message)
      (expect called-once? fake-list-channels-spy)))

  (expecting "calls post-message"
    (let [fake-post-message-spy (spy fake-post-message)]
      (share-channels/exec fake-list-channels fake-post-message-spy)
